  ...
  if (mode === 'choose') {
    return (
      <div className="max-w-2xl mx-auto px-4 py-16">
        <h1 className="text-2xl font-bold text-center mb-2">Let's get your item sold</h1>
        <p className="text-[var(--color-text-secondary)] text-center mb-10">Two ways to create your listing — pick what feels right</p>

        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <button
            onClick={() => setMode('ai')}
            className="p-6 rounded-xl bg-[var(--color-surface)] border border-[var(--color-accent)]/30 hover:border-[var(--color-accent)] text-left transition-colors"
          >
            <span className="text-2xl block mb-2">✦</span>
            <h3 className="font-semibold mb-1">Let AI Do the Work</h3>
            <p className="text-sm text-[var(--color-text-secondary)]">Upload photos and we'll write a scroll-stopping title, description, and price for you — instantly</p>
          </button>

          <button
            onClick={() => setMode('manual')}
            className="p-6 rounded-xl bg-[var(--color-surface)] border border-[var(--color-border)] hover:border-white/30 text-left transition-colors"
          >
            <span className="text-2xl block mb-2">✎</span>
            <h3 className="font-semibold mb-1">Write It Myself</h3>
            <p className="text-sm text-[var(--color-text-secondary)]">Full control over every detail, from the first word to the final price</p>
          </button>
        </div>
      </div>
    )
...
function AICreateFlow({ onBack }: { onBack: () => void }) {
  ...
  async function handleGenerate() {
    ...
    try {
      setStatus('uploading')
      setProgressLabel('Creating draft listing...')
      const draft = await listingsApi.create({ title: 'Draft in progress', condition })
      ...
      setStatus('queuing')
      setProgressLabel('Sending images for analysis...')
      const job = await listingsApi.aiGenerate(imageUrls, condition, draft.id)

      setStatus('polling')
      setProgressLabel('Analyzing photos, writing title and description, suggesting price...')
      ...
      const poll = setInterval(async () => {
        ...
        if (result.status === 'finished') {
          ...
          clearInterval(poll)
          setStatus('error')
          setErrorMsg('Analysis failed — try clearer photos or try again')
        }
      }, 2000)
    } catch {
      setStatus('error')
      setErrorMsg('An error occurred while uploading or analyzing your photos')
    }
  }
  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-16">
      <button onClick={onBack} className="text-sm text-[var(--color-text-secondary)] mb-6" disabled={isBusy}>← Back</button>
      <h1 className="text-2xl font-bold mb-6">Create with AI</h1>

      {status === 'idle' && (
        <>
          <div className="mb-6">
            <label className="text-sm text-[var(--color-text-secondary)] block mb-2">Product photos (up to 10)</label>
          ...
          <div className="mb-8">
            <label className="text-sm text-[var(--color-text-secondary)] block mb-2">Condition</label>
...
function ManualCreateFlow({ onBack }: { onBack: () => void }) {
  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-16">
      <button onClick={onBack} className="text-sm text-[var(--color-text-secondary)] mb-6">← Back</button>
      <h1 className="text-2xl font-bold mb-6">Create Listing Manually</h1>

      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        <Input label="Listing title" required value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g. iPhone 13 Pro, excellent condition" />
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <textarea
            ...
            placeholder="Describe the condition, accessories, reason for selling..."
          />
        </div>

        <Input label="Price (AUD)" type="number" required value={price} onChange={(e) => setPrice(e.target.value)} placeholder="0" />
