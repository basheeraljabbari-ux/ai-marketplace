...
# يستورد كل الـ SQLAlchemy models مرة وحدة قبل أي endpoint يشتغل — يمنع فشل
# ربط العلاقات (زي User.city -> City) لو Model معين ما استورده أي router مباشرة.
# نفس المبدأ المطبّق بـ alembic/env.py و tests/conftest.py بالضبط.
# (هذا الإصلاح تم التحقق منه فعلياً بالنشر الحي على Railway - راجع سجل المحادثة)
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # آمن هنا لأننا لا نستخدم cookies للمصادقة (Bearer tokens فقط)
